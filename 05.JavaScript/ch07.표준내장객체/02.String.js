...
console.log("총 " + totalSec + "초 입니다.");
let totalSeconds = 0;
for (let hour = 0; hour < 24; hour++) {
    for (let minute = 0; minute < 60; minute++) {
        const display = hour + ":" + minute;
        if (display.indexOf('3') >= 0)          //  디지털 시계에 3이 표시되면
            totalSeconds += 60;
    }
}
console.log(`총 ${totalSeconds}초 입니다.`);
console.log("\n-------------------- 구분선 --------------------\n");

// 정규 표현식
const sample = 'ㅠㅠA quick brown fox 갈색의 재빠른 여우가 Jumpes over 게으른 개를 The lazy dog.ㅋㅋ';

let newStr = sample.replace(/[A-Z]/, '대');     //  '[A-Z]', '/[A-Z]/'는 작동 안됨
console.log(newStr);
newStr = sample.replace(/o/, 'O');              //  한 개만 바꿈
console.log(newStr);
...
fruits = '사과, 배, 감, 포도';
fruitArr = fruits.split(', ');
console.log(fruitArr);

fruits = '사과, 배,감 , 포도';
fruitArr = fruits.split(', ');
console.log(fruitArr);
fruitArr = fruits.split(',').map(s => s.trim());    //  array element 각각에 대해서 callback 함수 적용
console.log(fruitArr);
console.log("\n-------------------- 구분선 --------------------\n");
...
console.log(today);
// 2022-12-01T06:32:46.369Z
console.log(today.substring(0,10));     //  날짜 추출: 2022-12-01
console.log(today.substring(11,19));    //  시간 추출: 06:32:46
console.log("\n-------------------- 구분선 --------------------\n");

// 현재 날짜/시간을 YYYY-MM-DD hh:mm:ss 형식으로 만들어주는ㄴ 함수
...
const now = dateTime();
console.log(now);                   //  2022-12-01 15:46:43
console.log(now.substring(2, 16));  //  22-12-01 15:47
console.log("\n-------------------- 구분선 --------------------\n");
